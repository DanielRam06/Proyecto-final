...
import java.awt.Dimension; // Importa la clase Dimension de AWT
import javax.swing.BorderFactory; // Importa la clase BorderFactory de Swing
...
private void components() {
 ...
 contentPane.setLayout(null); // Establece un diseño nulo para el panel principal
 contentPane.setBackground(Color.GRAY); // Establece el color de fondo del panel principal
 contentPane.setBounds(0, 0, width, height); // Establece las dimensiones del panel principal
...
private JSlider createColorSlider(Color color, int x, int y) {
 ...
 // Método que se ejecuta cuando se hace clic en el área de dibujo
    ...
    // Clase interna para manejar la acción de guardar la imagen
    private class SaveImageListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            fileChooser.setCurrentDirectory(new File(System.getProperty("user.home") + "/Desktop/")); // Establece el directorio actual del selector de archivos

            int returnVal = fileChooser.showSaveDialog(MyWindow.this); // Muestra el cuadro de diálogo de guardar archivo y guarda el resultado
            if (returnVal == JFileChooser.APPROVE_OPTION) { // Si el usuario aprueba la selección de archivos
                ...
                String filePath = file.getAbsolutePath(); // Obtiene la ruta absoluta del archivo seleccionado
                if (!filePath.endsWith(".jpg")) { // Si la ruta del archivo no termina con ".jpg"
                    file = new File(filePath + ".jpg"); // Agrega la extensión ".jpg" a la ruta del archivo
                }

                try {
                    // Crear una imagen con el tamaño del canvas y tipo RGB
                    ...
                    // Pintar el fondo del canvas en la imagen
                    g2d.setColor(canvas.getBackground());
                    g2d.fillRect(0, 0, canvas.getWidth(), canvas.getHeight());

                    // Pintar el contenido del canvas en la imagen
                    canvas.paint(g2d);
